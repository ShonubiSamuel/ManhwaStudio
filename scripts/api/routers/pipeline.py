...
# ══════════════════════════════════════════════════════════════════════════════
# Single dub-batch regeneration (targeted — NOT the whole dub stage)
# ══════════════════════════════════════════════════════════════════════════════
# Regenerating one batch is a precise operation: it re-runs only that batch's
# TTS, then invalidates Sync for ONLY that batch's panel range — across every
# language when the batch is English (the timing reference), or that language
# alone otherwise.  It deliberately does NOT use _cascade_downstream(), which is
# coarse (whole-language), so the panel-range precision the user asked for is
# preserved.

def _invalidate_sync_for_batch(
    db: Database, episode_id: int, lang: str, panel_indices: list[int],
) -> dict:
    """Clear synced clips for a batch's panel range and return a summary."""
    from pipeline_logic import clear_panel_sync

    ep = db.get_episode(episode_id) or {}
    if lang == "en":
        # English audio/timing changed → every other language must re-sync these
        # panels.  Other languages' dub audio is left untouched.
        targets = [c for c in config.SUPPORTED_LANGUAGES if c != "en"]
    else:
        targets = [lang]
    ...
    for pidx in panel_indices:
        ...
        if not panel:
            continue
        for code in targets:
            if db.get_panel_audio(panel["id"], code):
                clear_panel_sync(db, panel["id"], code)
                cleared += 1

    cur = ep.get("stage_sync")
    if cur in ("done", "running"):
        db.set_episode_stage(episode_id, "sync", "outdated")

    return {"langs": targets, "panels_cleared": cleared}


def _run_dub_batch_regen_thread(
    episode_id: int, lang: str, batch_idx: int, db: Database,
) -> None:
    import json
    ...
    label  = f"{config.SUPPORTED_LANGUAGES.get(lang, lang)} batch {batch_idx + 1}"
    ok     = False
    log_id = None
    try:
        tab._log(f"▶  Regenerating dub — {label} …", "accent")
        log_id = _log_start(db, episode_id, "dub")

        ep = db.get_episode(episode_id)
        if not ep:
            raise RuntimeError("Episode not found")

        # Resolve the voice profile (explicit assignment, else naming-convention).
        profiles = db.get_setting(f"dub_profiles_{episode_id}", {}) or {}
        if isinstance(profiles, str):
            try: profiles = json.loads(profiles)
            except Exception: profiles = {}
        vpm   = VoiceProfileManager(str(config.VOICES_DIR))
        name  = profiles.get(lang)
        if not name:
            match = [p for p in vpm.list_profiles() if p.lower().endswith(f"_{lang}")]
            name  = match[0] if match else None
        profile = vpm.load(name) if name else None
        if not profile:
            tab._log(f"No voice profile assigned for '{lang}' — set one in Dub, then retry.", "error")
            raise RuntimeError("missing voice profile")

        # Find the batch's panel range from the persisted batch state.
        state   = load_batch_state(get_batch_state_path(ep["output_folder"]))
        batch   = next((b for b in state.get(lang, {}).get("batches", [])
                        if b.get("idx") == batch_idx), None)
        if not batch:
            tab._log(f"Batch {batch_idx} not found for '{lang}'", "error")
            raise RuntimeError("batch not found")
        panel_indices = batch.get("panels") or list(range(batch["panel_from"], batch["panel_to"] + 1))

        engine = DubEngine(db, on_log=tab._log)
        tab._active_engine = engine
        ok = bool(engine.regenerate_batch(episode_id, lang, profile, batch_idx, on_log=tab._log))

        if ok:
            inv   = _invalidate_sync_for_batch(db, episode_id, lang, panel_indices)
            scope = "all languages" if lang == "en" else config.SUPPORTED_LANGUAGES.get(lang, lang)
            a, b  = batch["panel_from"] + 1, batch["panel_to"] + 1
            tab._log(
                f"↺  Sync invalidated for panels {a}–{b} · {scope} "
                f"({inv['panels_cleared']} clip(s) cleared) — re-run Sync to update",
                "warning",
            )
            tab._log(f"✓  {label} regenerated — continuous track rebuilt", "success")
        else:
            tab._log(f"✗  {label} regeneration failed", "error")

    except Exception as exc:
        tab._last_error = f"{exc}\n\n{traceback.format_exc()}"
        tab._log(f"✗  Dub batch regen crashed: {exc}", "error")
        ok = False

    finally:
        _log_end(db, log_id, success=ok, stopped=tab._stop_flag,
                 error=tab._last_error, lines=tab._log_lines)
        # NOTE: no _cascade_downstream — invalidation above is panel-range precise.
        emit_stage_done(episode_id, "dub", ok)
        with _state_lock:
            _running.pop(episode_id,     None)
            _active_tabs.pop(episode_id, None)


def launch_dub_batch_regen(episode_id: int, lang: str, batch_idx: int, db: Database) -> None:
    """Validate, clear stale events, and start the batch-regen thread (registered for /stop)."""
    _ensure_not_running(episode_id)
    clear_queue(episode_id)
    thread = threading.Thread(
        target = _run_dub_batch_regen_thread,
        args   = (episode_id, lang, batch_idx, db),
        daemon = True,
        name   = f"dubbatch-{lang}{batch_idx}-ep{episode_id}",
    )
    with _state_lock:
        _running[episode_id] = thread
    thread.start()
...
# ══════════════════════════════════════════════════════════════════════════════
# Fix "rushed" panels — per-panel re-translate (shorter) → re-dub → re-sync,
# best of N attempts.  Targets only panels whose dub is longer than the English
# budget by more than the comfort threshold (i.e. they get sped up).
# ══════════════════════════════════════════════════════════════════════════════
...
def _run_dub_fix_thread(episode_id: int, lang: str, indices, db: Database) -> None:
    from dub_engine import DubEngine
    ...
    log_id  = None
    try:
        log_id = _log_start(db, episode_id, "sync")
        ep = db.get_episode(episode_id)
        if not ep:
            raise RuntimeError("Episode not found")

        profile = _resolve_voice(db, episode_id, lang)
        if not profile:
            tab._log(f"No voice profile assigned for '{lang}' — set one in Dub, then retry.", "error")
            raise RuntimeError("missing voice profile")
        ...
        def shorten_fn(english, current, target):
            return translator.shorten_line(
                english, current, target, lang,
                provider=provider, api_key=api_key,
                lm_studio_model=lm_model, context_length=ctx, on_log=tab._log,
            )

        res = engine.fix_rushed_panels(
            episode_id, lang, profile, shorten_fn,
            comfort=comfort, attempts=3, floor=floor, indices=indices,
            on_log=tab._log, on_progress=tab._on_progress,
        )
        if res["targets"] == 0:
            tab._log(f"No rushed {lang} panels to fix — all within +{int((comfort - 1) * 100)}%.", "success")
        else:
            engine.stitch_synced(episode_id, lang, on_log=tab._log)
            tab._log(f"✓  Fixed {res['fixed']}/{res['targets']} {lang} panel(s) — "
                     f"best fit kept; combined track rebuilt", "success")
        success = True

    except Exception as exc:
        tab._last_error = f"{exc}\n\n{traceback.format_exc()}"
        tab._log(f"✗  Fix-rushed crashed: {exc}", "error")
        success = False
    finally:
        _log_end(db, log_id, success=success, stopped=tab._stop_flag,
                 error=tab._last_error, lines=tab._log_lines)
        emit_stage_done(episode_id, "sync", success)
        with _state_lock:
            _running.pop(episode_id,     None)
            _active_tabs.pop(episode_id, None)


def launch_dub_fix(episode_id: int, lang: str, indices, db: Database) -> None:
    """Start the per-panel fix-rushed job (registered for /stop)."""
    _ensure_not_running(episode_id)
    clear_queue(episode_id)
    thread = threading.Thread(
        target = _run_dub_fix_thread,
        args   = (episode_id, lang, indices, db),
        daemon = True,
        name   = f"dubfix-{lang}-ep{episode_id}",
    )
    with _state_lock:
        _running[episode_id] = thread
    thread.start()
