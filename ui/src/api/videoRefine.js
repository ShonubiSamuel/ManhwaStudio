...
/** List ONLY Video Refine projects (separate from Voiceover). */
export function listRefineProjects() {
  return get("/video-refine/projects")
}

/** Create a new Video Refine project (tagged so it never shows in Voiceover). */
export function createRefineProject(name, sourcePath = "") {
  return post("/video-refine/projects", { name, source_path: sourcePath })
}
